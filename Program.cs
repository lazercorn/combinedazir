    ...
    class Program
    {
        ...
        static void Harass()
        {
         ...
         // ObjectManager.Player.ServerPosition
                ...
                if (target.Distance(HeroManager.Player.ServerPosition) < 450)
                {
                ...
                else
                {
                    ...
                    {
                        if (useW)
                            W.Cast(HeroManager.Player.Position.Extend(target.ServerPosition, 450));
                    }
                }
                // if(azir.soldierManager.SoldiersAttackingn(azir)<=nSoldiersToQ)
                // StaticSpells.CastQ(azir, target, useQ, nSoldiersToQ);

                if(SoldiersManager.SoldiersAttackingn()<=nSoldiersToQ)
                StaticSpells.CastQ(azir, target, useQ, nSoldiersToQ);
                // SoldiersAttackingn
